...
class rfd extends Controller
{
    //
    function rfd(Request $req)
    {
        session_start(); 
        ...
        if($str == 1)
{
                DB::table('booking')->where('idbook', $username1)->update(array('status' => 'Ready for delievery')); 
                $contact_data = [] ;
                $contact_data['name'] = $name ;  
                $contact_data['email'] = $str1 ;   
                $contact_data['message'] = 'your Bike '.$bike.' is ready for delievery ' ;
